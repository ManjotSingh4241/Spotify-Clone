  ...
  var playAudio = storeService.playAudio;
  ...
  app.get('/shop/:id', async (req, res) => {
    ...
    try {
      ...
      const item = await Initialize.getItemById(itemId);

      // Pass the item's ID to the playAudio function
      playAudio(itemId);

      // Update the viewData object with the item
      viewData.item = item;
    } catch (err) {
      viewData.message = "no results";
    }
  ...
  app.post('/songs/add', upload.fields([
    { name: 'audioUrl', maxCount: 1 },
    { name: 'featureImage', maxCount: 1 }
  ]), function (req, res, next) {
    if (req.files && req.files['audioUrl'] && req.files['featureImage']) {
      let audioFile = req.files['audioUrl'][0];
      let imageFile = req.files['featureImage'][0];
  
      let audioUpload = new Promise((resolve, reject) => {
        cloudinary.uploader.upload(audioFile.path, (error, result) => {
          if (result) {
            resolve(result.url);
          } else {
            reject(error);
      ...
      Promise.all([audioUpload, imageUpload])
        .then(([audio, imageUrl]) => {
          req.body.audioUrl = audio;
